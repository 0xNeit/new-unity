...
interface UseExtractDataInput {
  pools: Pool[];
  vaults: Vault[];
  ucorePriceCents: BigNumber;
  uaiPriceCents: BigNumber;
}

const useExtractData = ({ pools, vaults, ucorePriceCents, uaiPriceCents }: UseExtractDataInput) =>
  useMemo(() => {
    ...
    const { totalVaultStakeCents, yearlyVaultEarningsCents } = vaults.reduce(
      (accTotalVaultStakeCents, vault) => {
        const vaultStakeCents = convertWeiToTokens({
          ...
          areTokensEqual(vault.stakedToken, TOKENS.ucore) ? ucorePriceCents : uaiPriceCents,
        );
  ...
  }, [pools, vaults, ucorePriceCents, uaiPriceCents]);
