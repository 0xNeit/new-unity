...
// We assume 1 UAI = 1 dollar
const UAI_PRICE_CENTS = new BigNumber(100);
...
export const AccountUi: React.FC<AccountUiProps> = ({ isFetching, vaults, pools }) => {
  ...
  const ucorePriceCents = useMemo(() => {
    let priceCents = new BigNumber(0);

    pools.forEach(pool =>
      pool.assets.every(asset => {
        if (areTokensEqual(asset.vToken.underlyingToken, TOKENS.ucore)) {
  ...
  return (
    <>
      <Summary
        ...
        ucorePriceCents={ucorePriceCents}
        uaiPriceCents={UAI_PRICE_CENTS}
